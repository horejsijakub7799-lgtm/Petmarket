...
const SLUZBY = ["Individuální venčení", "Skupinové venčení", "Denní hlídání", "Noční hlídání", "Krátkodobé hlídání", "Výcvik", "Transport k veterináři"];
const ZVIRATA = ["Malá plemena", "Střední plemena", "Velká plemena", "Všechna plemena"];

export default function VencitelRegister() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f7f4ef", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <nav style={{ background: "#fff", borderBottom: "1px solid #ede8e0", height: 64, display: "flex", alignItems: "center", padding: "0 32px", boxShadow: "0 1px 12px rgba(44,80,58,0.07)" }}>
        ...
        <div style={{ marginLeft: "auto", fontSize: "0.85rem", color: "#8a9e92" }}>Registrace venčitele psů</div>
      </nav>
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "40px 24px" }}>
        ...
        {step === 1 && (
          <div style={{ background: "#fff", borderRadius: 20, padding: "32px", boxShadow: "0 4px 20px rgba(44,80,58,0.08)" }}>
            <h1 style={{ fontFamily: "'DM Serif Display', serif", fontSize: "1.6rem", color: "#1c2b22", marginBottom: 6 }}>🦮 Registrace venčitele</h1>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
              <div style={{ background: "#f0f7f4", borderRadius: 12, padding: "16px 18px", border: "1px solid #b7d9c7" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  <div><label style={labelStyle}>Email *</label><input type="email" value={form.email} onChange={e => set("email", e.target.value)} placeholder="jana@vencim-psy.cz" style={inputStyle} /></div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
              <div>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 8 }}>
                  {SLUZBY.map(s => <button key={s} onClick={() => toggleSluzba(s)} style={toggleStyle(form.sluzby.includes(s))}>{s}</button>)}
                </div>
              </div>
              <div>
                <label style={labelStyle}>Přijímám plemena</label>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 8 }}>
                  {ZVIRATA.map(s => <button key={s} onClick={() => toggleZvire(s)} style={toggleStyle(form.zvirata.includes(s))}>{s}</button>)}
                </div>
              </div>
