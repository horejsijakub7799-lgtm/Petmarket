              ...
              <div key={h.id} onClick={() => setSelected(h)} style={{ background: "#fff", borderRadius: 16, overflow: "hidden", border: "1.5px solid #ede8e0", boxShadow: "0 2px 12px rgba(44,80,58,0.07)", cursor: "pointer", transition: "transform 0.2s, box-shadow 0.2s" }}
                ...
                <div style={{ padding: "16px 18px 20px" }}>
                  ...
                  <div style={{ fontSize: "0.82rem", color: "#8a9e92", marginBottom: 10, display: "flex", gap: 12 }}>
                    <span>📍 {h.city}</span>
                    {h.metadata?.capacity && <span>🐕 Kapacita: {h.metadata.capacity} psů</span>}
                  </div>
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 22, maxWidth: 560, width: "100%", maxHeight: "92vh", overflowY: "auto", boxShadow: "0 12px 40px rgba(44,80,58,0.14)" }}>
            ...
            <div style={{ padding: "24px 28px 28px" }}>
              ...
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 16 }}>
                {[`📍 ${selected.address}, ${selected.city}`, `📞 ${selected.phone}`, selected.web && `🌐 ${selected.web}`, selected.metadata?.capacity && `🐕 Kapacita: ${selected.metadata.capacity} psů`].filter(Boolean).map(tag => (
              ...
              {selected.metadata?.opening_hours && (
                <div style={{ marginBottom: 20 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4 }}>
                    {Object.entries(selected.metadata.opening_hours).map(([den, h]) => (
              ...
              <button onClick={() => selected.phone && window.open(`tel:${selected.phone}`)} style={{ width: "100%", background: "#2d6a4f", color: "#fff", border: "none", borderRadius: 10, padding: "13px", fontSize: "0.95rem", fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>📞 Zavolat hotelu</button>
            </div>
          </div>
        </div>
      )}
