...
export default function HotelyPage() {
  ...
  // Boosted hotely nahoře, zbytek podle data
  ...
  const HotelCard = ({ h }) => {
    ...
    return (
      <div key={h.id} onClick={() => navigate(`/partner/${h.id}`)}
        ...
        <div style={{ height: isBoosted ? 140 : 160, marginTop: isBoosted ? 30 : 0, background: "linear-gradient(145deg, #e8f5ef, #f2faf6)", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
          {h.foto_urls?.[0]
            ? <img src={h.foto_urls[0]} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
        ...
        <div style={{ padding: "16px 18px 20px" }}>
          ...
          {h.metadata?.sluzby?.length > 0 && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 10 }}>
              {h.metadata.sluzby.slice(0, 3).map(s => (
                <span key={s} style={{ background: "#e8f5ef", color: "#2d6a4f", borderRadius: 20, padding: "2px 9px", fontSize: "0.72rem", fontWeight: 600 }}>{s}</span>
              ))}
              {h.metadata.sluzby.length > 3 && (
                <span style={{ background: "#f7f4ef", color: "#8a9e92", borderRadius: 20, padding: "2px 9px", fontSize: "0.72rem" }}>+{h.metadata.sluzby.length - 3}</span>
              )}
            </div>
          )}
