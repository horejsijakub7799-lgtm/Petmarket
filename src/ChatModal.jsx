...
// Filtr kontaktních informací
function filterMessage(text) {
  const patterns = [
    /\b[\w.-]+@[\w.-]+\.\w{2,}\b/gi, // email
    /(\+420|00420)?\s?[0-9]{3}\s?[0-9]{3}\s?[0-9]{3}/g, // CZ telefon
    /\b\d{9,}\b/g, // dlouhé číslo
    /https?:\/\/[^\s]+/gi, // URL
    /www\.[^\s]+/gi, // www
    /instagram|facebook|whatsapp|telegram|signal|viber|skype/gi, // soc. sítě
    /ig:|fb:|wa:|tg:/gi, // zkratky
  ];
  let filtered = text;
  patterns.forEach(p => {
...
export default function ChatModal({ item, onClose }) {
  ...
  const receiverId = item.seller_id;
  const isOwner = user?.id === receiverId;
  ...
  const sendMessage = async () => {
    if (!text.trim()) return;
    if (!user) { setWarning("Musíš být přihlášen."); return; }

    const filtered = filterMessage(text.trim());
    if (filtered !== text.trim()) {
      setWarning("⚠️ Zpráva obsahuje kontaktní údaje které nejsou povoleny. Použij prosím chat pro komunikaci.");
      return;
    }
    ...
    const { error } = await supabase.from("messages").insert({
      inzerat_id: item.id,
      sender_id: user.id,
      receiver_id: isOwner ? messages.find(m => m.sender_id !== user.id)?.sender_id : receiverId,
            ...
            return (
              <div key={msg.id} style={{ display: "flex", justifyContent: isMe ? "flex-end" : "flex-start" }}>
                <div style={{
                  ...
                  {!isMe && (
                    <div style={{ fontSize: "0.72rem", fontWeight: 600, color: "#2d6a4f", marginBottom: 4 }}>
                      {msg.sender_name}
                    </div>
                  )}
                  <div>{msg.content}</div>
